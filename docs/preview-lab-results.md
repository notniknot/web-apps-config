...
- Gateway API HTTPRoute through KGateway: in progress. The route applies inside the vCluster, but host-side HTTPRoute sync has not appeared yet. Argo CD `ServerSideDiff=true` on the preview Application caused a comparison error for `gateway.networking.k8s.io/v1, Kind=HTTPRoute` inside the vCluster and should be avoided for these preview apps.
